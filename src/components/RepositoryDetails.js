...
function RepositoryDetails() {
  ...
  if (error) return <ErrorMessage message={error} onRetry={fetchRepositoryDetails} />;
  if (!repository) return <ErrorMessage message="Repository not found" onRetry={() => navigate('/')} retryText="Back to List" />;
  ...
  return (
    <div className="repo-details">
      ...
      <div className="repo-additional-info">
        <h3 className="repo-additional-title">Additional Information</h3>
        <div className="repo-additional-grid">
          ...
          {repository.license && <div><strong>License:</strong> {repository.license.name}</div>}
        </div>
      </div>
    </div>
  );
